...
function IncomingRequests(currUser : User,  incoming: Array<User> | null) {
  ...
  const users = incoming.length == 0 ? <div><h2>No Incoming Requests</h2></div>
    : 
...
function OutgoingRequests(user : User, outgoing: Array<User> | null) {
  ...
  const users = outgoing.length == 0 ? <div><h2>No Outgoing Requests</h2></div> 
    : <div className="userBlock">
...
async function decline(request : User) {
  if (
    confirm("Are you sure you want to decline " + request.profile.userName + "'s request ?")
  ) {
...
async function accept(request : User) {
  if (
    confirm("Are you sure you want to accept " + request.profile.userName + "'s request ?")
  ) {
    if (request !== undefined) {
      const indexRequest = fp.user.incomingRequests.indexOf(request.userid, 0);
      indexRequest > -1 ? fp.user.incomingRequests.splice(indexRequest, 1) : null;
...
async function cancel(request: User) {
  if (
    confirm("Are you sure you want to cancel your friend request to " + request.profile.userName + "?")
  ) {
    if (request !== undefined) {
      ...
      indexRequest > -1 ? fp.user.outgoingRequests.splice(indexRequest, 1) : null;
